 ...
 *
 * Run: bunx tsx scripts/cycle-001/publish.ts
 */
...
async function main() {
  const videoPath = process.env.CYCLE_001_VIDEO_PATH!
  const title = process.env.CYCLE_001_TITLE!
  const description = process.env.CYCLE_001_DESCRIPTION!
  const tags = JSON.parse(process.env.CYCLE_001_TAGS ?? '[]') as string[]
  const category = process.env.CYCLE_001_CATEGORY ?? '27'
  const privacy = (process.env.CYCLE_001_PRIVACY ?? 'private') as 'private' | 'unlisted' | 'public'
  const manifestPath = process.env.CYCLE_001_MANIFEST!
  ...
  let pillar = await db.contentPillar.findFirst({ where: { name: 'Autonomous Cycle' } })
  if (!pillar) {
