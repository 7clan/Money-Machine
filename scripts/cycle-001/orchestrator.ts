 ...
 *
 * Every stage writes a JSON artifact to data/autonomous-runs/cycle-001/ (audit trail).
 ...
 *
 * Run: bunx tsx scripts/cycle-001/orchestrator.ts
 */
...
function runSubagent(stage: string, agentFile: string, input: unknown, instance = 'default'): unknown {
  ...
  const result = spawnSync('bunx', ['tsx', path.join('src', 'agents', `${agentFile}.ts`)], {
    env: { ...process.env, SUBAGENT_CHAIN_DIR: chainDir, SUBAGENT_FLOW: 'cycle-001', SUBAGENT_BATCH: stage, SUBAGENT_INSTANCE: instance },
...
function runSubagentParallel(stage: string, agentFile: string, inputs: { instance: string; input: unknown }[]): Promise<unknown[]> {
  const tasks = inputs.map(({ instance, input }) => {
    ...
    mkdirSync(path.join(chainDir, 'tmp'), { recursive: true })
    writeFileSync(path.join(chainDir, 'input.json'), `${JSON.stringify(input, null, 2)}\n`)
    return { instance, chainDir }
  })
  log(`SUBAGENT PARALLEL START: ${agentFile} n=${tasks.length} stage=${stage}`)
  const startedAt = Date.now()
  const children = tasks.map(({ instance, chainDir }) => {
    ...
    child.stdout.on('data', (c: Buffer) => { stdoutChunks.push(c); writeFileSync(path.join(chainDir, 'stdout.log'), Buffer.concat(stdoutChunks)) })
    child.stderr.on('data', (c: Buffer) => { stderrChunks.push(c); writeFileSync(path.join(chainDir, 'stderr.log'), Buffer.concat(stderrChunks)) })
    return { instance, chainDir, child, promise: new Promise<unknown>((resolve, reject) => {
      child.on('close', (code: number | null) => {
        const dur = Date.now() - startedAt
        if (code !== 0) {
          log(`SUBAGENT PARALLEL FAIL: ${agentFile} instance=${instance} exit=${code} dur=${dur}ms`)
          reject(new Error(`${agentFile}[${instance}] failed (exit ${code})`))
        } else {
          try {
            const out = JSON.parse(readFileSync(path.join(chainDir, 'output.json'), 'utf8'))
            log(`SUBAGENT PARALLEL PASS: ${agentFile} instance=${instance} dur=${dur}ms hash=${sha(out)}`)
            resolve(out)
          } catch (e) {
  ...
  return Promise.all(children.map((c) => c.promise))
}
...
function runProduction(script: any, shots: any[]): { videoPath: string; assetManifest: any; audioManifest: any; compositionHash: string; durationSec: number } {
  ...
  const start = Date.now()
  const result = spawnSync('bunx', ['tsx', path.join('scripts', 'cycle-001', 'produce.ts')], {
...
async function runPublishing(videoPath: string, titleThumb: any, script: any): Promise<{ youtubeVideoId: string; uploadStatus: string }> {
  ...
  const r = spawnSync('bunx', ['tsx', path.join('scripts', 'cycle-001', 'publish.ts')], {
    env: { ...process.env, CYCLE_001_VIDEO_PATH: videoPath, CYCLE_001_TITLE: meta.title, CYCLE_001_DESCRIPTION: meta.description, CYCLE_001_TAGS: JSON.stringify(meta.tags), CYCLE_001_CATEGORY: meta.category, CYCLE_001_PRIVACY: meta.privacy, CYCLE_001_MANIFEST: path.join(CYCLE_DIR, 'publish-manifest.json') },
    stdio: ['ignore', 'pipe', 'pipe'],
    timeout: 300_000,
    encoding: 'utf8',
  })
  ...
  if (r.status !== 0) {
    ...
    recordStage('PublishingAgent', 'FAIL', Date.now() - start)
    throw new Error(`publishing worker failed (exit ${r.status})`)
  }
...
async function main() {
  ...
  try {
    ...
    // RESUME for Stage 8: if fact-check.json exists, reuse it
    let factCheck: any = loadArtifact('fact-check.json')
    if (factCheck) {
      log(`RESUME: reusing fact-check.json (verdict=${factCheck.verdict})`)
      summary.factCheckHash = sha(factCheck)
      recordStage('FactChecker', 'PASS', undefined, summary.factCheckHash, `RESUMED verdict=${factCheck.verdict}`)
    } else {
      factCheck = runFactChecker(script as any)
      if (factCheck.verdict === 'FAIL') {
        log(`FactChecker FAIL — proceeding to QC; will repair.`)
      }
    }
    // RESUME for Stage 9: if qc-round-1.json exists, reuse it
    ...
    // RESUME for Stage 11a: creative-lock.json
    ...
    // RESUME for Stage 11b: title-thumbnail.json
    ...
    const publish = await runPublishing(finalVideo, titleThumb as any, script as any)
    log(`PUBLISHED: videoId=${publish.youtubeVideoId} privacy=private`)
    summary.analytics = 'PENDING_DATA'
    recordStage('AnalyticsAgent', 'PENDING_DATA', undefined, undefined, 'Brand-new private video — no analytics data yet. yt-analytics.readonly scope upgrade required.')
    summary.overall = 'PASS'
    summary.finishedAt = ts()
    writeArtifact('run-summary.json', summary)
    writeArtifact('analytics-status.json', { status: 'PENDING_DATA', reason: 'Brand-new private video. yt-analytics.readonly scope not yet authorized by user. Authorization flow exposed in UI.', videoId: summary.videoId })
    log('==================================================')
    log('AUTONOMOUS CYCLE 001 — PASS')
    log(`videoId=${summary.videoId} privacy=private`)
    log('==================================================')
  } catch (e) {
