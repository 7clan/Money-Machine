 ...
 *
 * Run: bunx tsx scripts/capability-showcase-001/thumbnail.ts
 */
...
async function main() {
  ...
  // 4. ATTEMPT YouTube thumbnail upload (only if publish-manifest.json exists with a videoId)
  ...
  if (existsSync(publishManifestPath)) {
    try {
      ...
      videoId = publishManifest.youtubeVideoId || null
    } catch {
      videoId = null
    }
  }
  if (!videoId) {
    log('No videoId available yet (publish-manifest.json missing or no youtubeVideoId) — marking upload NOT_ATTEMPTED. Creation + QC still PASS.')
    const manifest = { creationStatus: 'PASS', qcStatus: 'PASS', uploadStatus: 'NOT_ATTEMPTED', thumbnailPath: THUMB_PATH, qc, finishedAt: new Date().toISOString() }
    writeFileSync(MANIFEST_PATH, `${JSON.stringify(manifest, null, 2)}\n`)
    log('Thumbnail manifest: creation=PASS qc=PASS upload=NOT_ATTEMPTED')
    await db.$disconnect()
    return
  }
